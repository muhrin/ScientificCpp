...
\begin{frame}[fragile]
  ...
  \newline\pause
  Sometimes friendship can be used effectively to keep dependent code separated if there is a good design reason.  For example the code to draw a rectangle may be very complicated and may fit more naturally in the graphics handling portion of the codebase.
  }
...
\begin{frame}
  ...
  \begin{itemize}
  	\item<2->{I can't access my friend's \kw{private}s their unless they mark me as a \kw{friend}.}
    \item<3->{Friends of friends can't access my \kw{private}s.}
    \item<4->{My children can't access my \kw{private}s.}
    \item<5->{I can't access my children's \kw{private}s.}
  \end{itemize}
