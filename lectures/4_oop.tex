...
\subtitle{Session 4: Object oriented programming}
...
\begin{frame}[fragile]
  ...
  \begin{columns}[t]
    \begin{column}[T]{0.5\linewidth}
      ...
      {../code/4_oop/lectures/simple_vector2.cpp}
    \end{column}
    \pause
    \begin{column}[T]{0.5\linewidth}
...
\begin{frame}[fragile]
  ...
  \end{tabularx}
  \uncover<5->{By default classes have \kw{private} access to all members.}
\end{frame}
...
\begin{frame}[fragile]
  ...
  Encapsulation separates implementation details from the way the class is used, this is sevral advantages:
  \pause
  \begin{itemize}
    ...
    \pause
    \item{Developing complex code can be easier as a common set of iterfaces can be defined and then implemented separately in due time.  So long as each class fulfills its responsibilities the whole will work.}
  \end{itemize}
  \pause
...
int main()
{
  ...
  \end{lstlisting}
  \pause
  But using \texttt{(*name).member} can get a bit annoying.  Kindly, C++ provides us with an alternative: the indirection operator (\texttt{->}).\pause{}  One more:
  \begin{lstlisting}
int main()
{
  Vector2 * r1 = new Vector2;
...
\begin{frame}[fragile]
  ...
  r2.operator -(r1);
    \end{lstlisting}}
    \pause
    \item{Now it's more obvious: \texttt{r2}'s \texttt{operator -} method is invoked with \texttt{r1} as a paramter.}
    \pause
